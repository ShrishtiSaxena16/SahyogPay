...
const Notifications = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/borrower-notifications/${userId}`);
  ...
  const handleMarkAsRead = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5000/api/borrower-notifications/${userId}`, 
      { status: 'read' }, 
